...
struct CloudDb { conn: Connection }
...
impl CloudDb {
    ...
    fn save(&self, r: &VmRecord) -> Result<i64> {
        self.conn.execute(
            ...
            params![r.provider, r.vm_id, r.name, r.region, r.size, r.ip, r.status, r.created_at],
        )?;
    ...
    fn list(&self) -> Result<Vec<VmRecord>> {
        let mut stmt = self.conn.prepare(
            &format!("{SELECT_ALL} ORDER BY created_at DESC")
        )?;
    ...
    fn find(&self, query: &str) -> Result<Option<VmRecord>> {
        if let Ok(id) = query.parse::<i64>() {
            let r = self.conn.query_row(
                &format!("{SELECT_ALL} WHERE id=?1"), params![id], map_row,
            ).optional()?;
            if r.is_some() { return Ok(r); }
        }
        self.conn.query_row(
            &format!("{SELECT_ALL} WHERE name=?1 OR vm_id=?1"),
            params![query], map_row,
        ).optional().map_err(Into::into)
    }

    fn remove(&self, id: i64) -> Result<()> {
        self.conn.execute("DELETE FROM cloud_vms WHERE id=?1", params![id])?;
        Ok(())
    }

    fn count(&self) -> Result<u32> {
        self.conn.query_row("SELECT COUNT(*) FROM cloud_vms", [], |r| r.get(0))
            .map_err(Into::into)
    }
}
...
fn map_row(row: &rusqlite::Row) -> rusqlite::Result<VmRecord> {
    Ok(VmRecord {
        id: row.get(0)?, provider: row.get(1)?, vm_id: row.get(2)?,
        name: row.get(3)?, region: row.get(4)?, size: row.get(5)?,
        ip: row.get(6)?, status: row.get(7)?, created_at: row.get(8)?,
    })
}
...
pub async fn run(action: CloudAction) -> Result<()> {
    match action {
        CloudAction::Up { provider, size, region, image, name } => {
            up(&provider, size.as_deref(), region.as_deref(), image.as_deref(), name.as_deref()).await
        }
        CloudAction::Ls          => ls(),
...
async fn up(
    ...
    let record = VmRecord {
        id: 0, provider: provider_str.to_string(), vm_id,
        name: spec.name, region: spec.region, size: spec.size, ip,
...
fn resolve_spec(
    ...
    VmSpec {
        ...
        region: region.unwrap_or_else(|| provider.default_region()).to_string(),
        image:  image.unwrap_or_else(|| provider.default_image()).to_string(),
...
async fn down(vm: &str) -> Result<()> {
    ...
    db.remove(record.id)?;
    println!("✓  Destroyed {} and removed from local records.", record.name);
    Ok(())
}
...
fn ssh(vm: &str) -> Result<()> {
    ...
    let ip = record.ip.as_deref().ok_or_else(|| anyhow::anyhow!(
        "`{}` has no IP yet — still provisioning. Retry in ~30s.", record.name
    ))?;
...
async fn provision_do(spec: &VmSpec, token: &str) -> Result<(String, Option<String>)> {
    ...
    let vm_id = json["droplet"]["id"]
        .as_u64().ok_or_else(|| anyhow::anyhow!("No droplet ID in response"))?.to_string();
    let ip = json["droplet"]["networks"]["v4"]
        .as_array().and_then(|a| a.first())
        .and_then(|n| n["ip_address"].as_str()).map(String::from);
    Ok((vm_id, ip))
}
...
async fn provision_hetzner(spec: &VmSpec, token: &str) -> Result<(String, Option<String>)> {
    ...
    let vm_id = json["server"]["id"]
        .as_u64().ok_or_else(|| anyhow::anyhow!("No server ID in response"))?.to_string();
    let ip = json["server"]["public_net"]["ipv4"]["ip"].as_str().map(String::from);
    Ok((vm_id, ip))
}
...
fn provision_aws(spec: &VmSpec) -> Result<(String, Option<String>)> {
    ...
    if !out.status.success() {
        bail!("aws ec2 run-instances failed: {}", String::from_utf8_lossy(&out.stderr));
    }
    let json: serde_json::Value = serde_json::from_slice(&out.stdout)?;
    let vm_id = json["Instances"][0]["InstanceId"]
        .as_str().ok_or_else(|| anyhow::anyhow!("No InstanceId in response"))?.to_string();
    let ip = json["Instances"][0]["PublicIpAddress"]
        .as_str().filter(|s| !s.is_empty()).map(String::from);
    Ok((vm_id, ip))
}
...
fn destroy_aws(vm_id: &str, region: &str) -> Result<()> {
    let out = std::process::Command::new("aws")
        ...
        .map_err(|e| anyhow::anyhow!("Cannot run `aws`: {e}"))?;
    if !out.status.success() {
        bail!("aws ec2 terminate-instances failed: {}", String::from_utf8_lossy(&out.stderr));
    }
    Ok(())
}
...
fn provision_azure(spec: &VmSpec) -> Result<(String, Option<String>)> {
    ...
    if !out.status.success() {
        bail!("az vm create failed: {}", String::from_utf8_lossy(&out.stderr));
    }
    let json: serde_json::Value = serde_json::from_slice(&out.stdout)?;
    let ip = json["publicIpAddress"].as_str().filter(|s| !s.is_empty()).map(String::from);
...
fn ensure_azure_resource_group(location: &str) -> Result<()> {
    let out = std::process::Command::new("az")
        ...
        .map_err(|e| anyhow::anyhow!("Cannot run `az`: {e}"))?;
    if !out.status.success() {
        bail!("Failed to create resource group: {}", String::from_utf8_lossy(&out.stderr));
    }
    Ok(())
}

fn destroy_azure(vm_id: &str) -> Result<()> {
    let (rg, name) = vm_id.split_once('/')
    ...
    let out = std::process::Command::new("az")
        ...
        .map_err(|e| anyhow::anyhow!("Cannot run `az`: {e}"))?;
    if !out.status.success() {
        bail!("az vm delete failed: {}", String::from_utf8_lossy(&out.stderr));
    }
    Ok(())
}
...
fn provision_gcp(spec: &VmSpec) -> Result<(String, Option<String>)> {
    ...
    let out = std::process::Command::new("gcloud")
        .args(["compute", "instances", "create", &spec.name,
               "--machine-type",  &spec.size,
               "--zone",          &spec.region,
               "--image-family",  &img_family,
               "--image-project", &img_project,
               "--labels",        "created-by=dev-claw",
               "--format",        "json"])
        .output()
        .map_err(|e| anyhow::anyhow!("Cannot run `gcloud`: {e}"))?;
    if !out.status.success() {
        bail!("gcloud instances create failed: {}", String::from_utf8_lossy(&out.stderr));
    }
    let json: serde_json::Value = serde_json::from_slice(&out.stdout)?;
    let vm_id = json[0]["name"].as_str()
        .ok_or_else(|| anyhow::anyhow!("No instance name in GCP response"))?.to_string();
...
fn destroy_gcp(vm_id: &str, zone: &str) -> Result<()> {
    let out = std::process::Command::new("gcloud")
        .args(["compute", "instances", "delete", vm_id,
               "--zone",  zone,
               "--quiet"])
        .output()
        .map_err(|e| anyhow::anyhow!("Cannot run `gcloud`: {e}"))?;
    if !out.status.success() {
        bail!("gcloud instances delete failed: {}", String::from_utf8_lossy(&out.stderr));
    }
    Ok(())
}
...
async fn provision(provider: &Provider, spec: &VmSpec, token: &str) -> Result<(String, Option<String>)> {
    match provider {
...
mod tests {
    ...
    fn sample_record(name: &str, vm_id: &str) -> VmRecord {
        VmRecord {
            id: 0, provider: "do".into(), vm_id: vm_id.into(),
            name: name.into(), region: "nyc3".into(), size: "s-1vcpu-1gb".into(),
            ip: Some("1.2.3.4".into()), status: "active".into(),
    ...
    fn cfg_with_cloud_limit(limit: u32) -> Config {
        Config {
            stack: None, doctor: None, git: None, workflows: None,
            usage: Some(UsageLimits {
                ...
                cloud_limit: Some(limit), warn_at_percent: None, reset_day: None,
            }),
        }
    }
    ...
    fn resolve_spec_custom_overrides_defaults() {
        let spec = resolve_spec(&Provider::Aws, Some("m5.large"), Some("eu-west-1"), None, Some("x"));
    ...
    fn cloud_db_find_missing_returns_none() {
        assert!(CloudDb::open_memory().unwrap().find("none").unwrap().is_none());
    }
    ...
    fn check_vm_limit_blocks_at_limit() {
        let db = CloudDb::open_memory().unwrap();
        for i in 0..3 {
            db.save(&sample_record(&format!("vm{i}"), &i.to_string())).unwrap();
        }
